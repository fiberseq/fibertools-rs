...
fn build_haplotype_map(header: &bam::Header, hap1_tag: Option<&str>, hap2_tag: Option<&str>) -> Result<Option<HashMap<i32, Option<u8>>>> {
...
fn add_haplotype_tag(record: &mut bam::Record, haplotype_map: &HashMap<i32, Option<u8>>, min_mapq: u8) -> Result<()> {
...
pub fn run_pg_pansn(opts: &mut PgPansnOptions) -> Result<()> {
    ...
    if let Some(ref prefix) = opts.prefix {
        ...
        log::info!("Stripping panSN-spec with delimiter: {delimiter}", delimiter = opts.delimiter);
        opts.input.strip_pansn_spec(opts.delimiter);
    }

    // Build haplotype mapping if haplotag options are provided
    let haplotype_map = build_haplotype_map(&opts.input.header(), opts.hap1_tag.as_deref(), opts.hap2_tag.as_deref())?;
