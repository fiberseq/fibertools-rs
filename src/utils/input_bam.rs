...
pub struct InputBam<C: CallableArgs = NoCallableArgs> {
    ...
    ///
    /// Fiber-seq tags describe the full read, so aligned input must be soft-clipped: pbmm2 never hard-clips; for ONT use `dorado aligner --mm2-opts "-Y"` or `minimap2 -Y -y`. Tags on hard-clipped records are dropped with a warning.
